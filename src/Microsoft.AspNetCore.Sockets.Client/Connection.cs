...
namespace Microsoft.AspNetCore.Sockets.Client
{
    public class Connection : IChannelConnection<Message>
    {
        ...
        private Connection(Uri url, ITransport transport, IChannelConnection<Message> toFromTransport, ILogger logger)
        {
            ...
            _toFromTransport.Input.Completion.ContinueWith(t =>
            {
                if (t.IsFaulted)
                {
                    _toFromTransport.Output.TryComplete(t.Exception);
                }
            });
        }

        public IReadableChannel<Message> Input => _toFromTransport.Input;
        public IWritableChannel<Message> Output => _toFromTransport.Output;

        public void Dispose()
        {
            _toFromTransport.Dispose();
            _transport.Dispose();
        }
